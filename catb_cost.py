...
# 加载数据
data = pd.read_csv('price359_outlier.csv')
X = data.iloc[:, 1:-3]
y = data.iloc[:, -1]
...
plt.xlabel('Actual Cost [$/mol]', fontsize=22)
plt.ylabel('Predicted Cost [$/mol]', fontsize=22)
...
plt.tight_layout()
plt.savefig('actual_vs_predicted_cost.png', dpi=600, bbox_inches='tight')
plt.show()

# 保存最终模型
final_model.save_model('best_catboost_model.cbm')
joblib.dump(final_model, 'model_cost.pkl')
...
plt.savefig('shap_feature_importance_cost.png', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig('shap_cost.png', dpi=600, bbox_inches='tight')
plt.show()

# 修复：定义selected_shap_values和selected_features
if X_train_full.shape[1] >= 35:
    selected_shap_values = shap_values[:, 20:28]
    selected_features = X_train_full.iloc[:, 20:28]
    ...
    plt.savefig('shap_selected_features_cost.png', dpi=600, bbox_inches='tight')
    plt.show()
else:
