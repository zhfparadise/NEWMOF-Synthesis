...
# 加载数据
data = pd.read_csv('递归式特征消除SSA20.csv')
...
plt.savefig('shap_feature_importance_SSA20.png', dpi=600)  # 更高的分辨率
plt.show()

# 绘制SHAP值的汇总图，控制最大显示的特征数量
plt.figure(figsize=(6, 6))  # 更大的图像尺寸
shap.summary_plot(shap_values, X_train, show=False, max_display=11)  # 显示前15个重要特征
plt.savefig('shap_SSA20_M+L.png', dpi=600)  # 更高的分辨率
plt.show()