...
# 保存模型为joblib格式
joblib.dump(model, 'model_surface_area.pkl')
...
shap.summary_plot(shap_values, X_train, plot_type="bar", show=False, max_display=38)
plt.savefig('shap_feature_importance_GSA.png', dpi=600)  # 更高的分辨率
plt.show()
...
plt.figure(figsize=(6, 6))  # 更大的图像尺寸
shap.summary_plot(shap_values, X_train, show=False, max_display=19)  # 显示前15个重要特征
plt.savefig('shap_GSA.png', dpi=600)  # 更高的分辨率
plt.show()
...
plt.savefig('shap_selected_features_SSA.png', dpi=600)  # 保存图片
plt.show()