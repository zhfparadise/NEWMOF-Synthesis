...
# ========== 1. 读取数据 ==========
P_df = pd.read_csv('输入数据PV.csv')  # 全部正例数据
PV_df = pd.read_csv('pareto_optimal_solutions_pv.csv')  # 待测试数据
...
noise = np.random.normal(loc=0.0, scale=0.6, size=U_data_raw.shape)
...
plot_score_distribution(
    ...
    filename='crystal_score_P_test_PV.png'
)
...
plot_score_distribution(
    ...
    color='slategray',
    xlabel='Crystal Score',
    filename='crystal_score_U_PV.png'
)
...
plot_score_distribution(
    ...
    color='steelblue',
    xlabel='Crystal Score',
    filename='crystal_score_PV.png'
)

# ========== 13. 保存结果 ==========
pd.DataFrame({'Positive_Test_Score': P_test_score}).to_csv('P_test_score_PV.csv', index=False)
pd.DataFrame({'Unlabeled_Score': U_score}).to_csv('U_score.csv', index=False)
pd.DataFrame({'PV_Best_Score': SSA_score}).to_csv('PV_score.csv', index=False)
