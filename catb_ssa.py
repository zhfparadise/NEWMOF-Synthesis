...
plt.xlabel('Actual SSA [m²/g]', fontsize=22)
plt.ylabel('Predicted SSA [m²/g]', fontsize=22)
...
plt.tight_layout()
plt.savefig('actual_vs_predicted.png', dpi=600, bbox_inches='tight')
plt.show()

# 保存最终模型
final_model.save_model('best_catboost_model.cbm')
joblib.dump(final_model, 'model_surface_area.pkl')
...
plt.savefig('shap_feature_importance_SSA.png', dpi=600, bbox_inches='tight')
plt.show()
...
plt.savefig('shap_SSA.png', dpi=600, bbox_inches='tight')
plt.show()
...
if X_train_full.shape[1] >= 35:
    ...
    plt.savefig('shap_selected_features_SSA.png', dpi=600, bbox_inches='tight')
    plt.show()
else:
