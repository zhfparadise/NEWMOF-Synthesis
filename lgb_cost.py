...
# 手动设置LightGBM超参数
params = {
    'n_estimators': 900,  # 树的数量
    'max_depth': 7,  # 树的最大深度
    'learning_rate': 0.01,  # 学习率
    'subsample': 0.9,  # 采样比例
    'colsample_bytree': 0.9,  # 特征采样比例
    'min_child_weight': 0.3,  # 类似XGBoost的gamma
    'reg_alpha': 3,  # L1正则
    'reg_lambda': 3,  # L2正则
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train, y_train), 1):
    ...
    model = lgb.train(
        ...
        callbacks=[
            ...
            lgb.log_evaluation(period=100)  # 每100轮输出一次评估结果
        ]
    )
...
test_data_full = lgb.Dataset(X_test, label=y_test, reference=train_data_full)
...
final_model = lgb.train(
    ...
    train_data_full,
    num_boost_round=params['n_estimators'],
    valid_sets=[train_data_full, test_data_full],
    callbacks=[
...
plt.legend()
plt.savefig('lgb_cost_with_cv.png', dpi=300, bbox_inches='tight')
plt.show()

# 保存最终模型
joblib.dump(final_model, 'best_lightgbm_model_with_cv.pkl')
print("\n模型已保存为 'best_lightgbm_model_with_cv.pkl'")